...
export default function HomeClient({
  ...
  // Fetch categories client-side (will use initial data on first render, then refetch if store changes)
  ...
  // Use tRPC data if available (client navigated/store changed), otherwise use initial SSR data
  const products = data?.pages.flatMap((page) => page.products) ?? initialProducts
  const categories = categoriesData ?? initialCategories

  const currentStoreSlug = stores.find(s => s.id === currentStoreId)?.slug || initialStoreSlug || 'store'
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {products.map((product) => (
                  <ProductCard
                    key={product.id}
                    product={{ ...product, store: product.store ?? undefined }}
                    storeSlug={product.store?.slug || currentStoreSlug}
                  />
                ))}
              </div>
